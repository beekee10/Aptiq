...
const Appointments = () => {
  ...
  const {feeSymbol}=useContext(AppContext)
  ...
  const getAvailableSlots=async ()=>{
    ...
    for(let i=0;i<7;i++){
      ...
      while(currentDate<endTime){
        ...
        //Incrrement current time by 30 minutes
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 -mt-20 sm:mt-0'>
          ...
          <p className='text-gray-500 font-medium mt-4'>
            Appointment fee : <span className='text-gray-600'>{docInfo.fees}{feeSymbol}</span>
          </p>
        </div>
      </div>
         {/*----- Booking slots ---------*/}
         <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
            ...
            <div className='flex items-center w-full gap-2 overflow-x-scroll mt-4'>
              {
                docSlots.length && docSlots[SlotIndex].map((item,index)=>(
                  <p onClick={()=>{setSlotTime(item.time)}} className={`text-sm font-light shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime? 'bg-blue-500 text-white':'text-gray-400 border border-gray-300' } `} key={index}>
